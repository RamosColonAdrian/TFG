...
const AddZone = (props: Props) => {
    ...
    async function createUserToZone() {
        selectedUsers.forEach(userId => {
            axios.post('http://localhost:8007/add-zone-user-to-zone', {
                    zone,
                    userId
            });
        });
                ...
                <div className="grid gap-6 mb-6 lg:grid-cols-2 w-full">
                    ...
                    <div>
                        {selectedUsers.map((user) => (
                            <div key={user}>
                                <p className='mb-2 text-gray-600' >&emsp;&emsp;- {getUser(user)} <span onClick={() => handlerDeleteUser(user)} className='text-lg cursor-pointer font-bold' >x</span> </p>
                            </div>
                        ))}
