...
const DepartamentList: React.FC = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8007/departments-and-users')
      .then((response) => {
                ...
                <tbody className="text-gray-600 text-sm font-light">
                  {departments.map((depart) => (
                    <tr className="border-b border-gray-200 bg-gray-50 hover:bg-gray-100">
                      ...
                      <td className="py-3 px-6 text-center">
                        <span className="font-medium">{depart.description}</span>
                      </td>
                      <td className="py-3 px-6 text-center">
                        {depart.User.length === 0
                          ...
                          <div className="flex items-center justify-center">
                            {depart.User.map((user, index) => (
                              ...
                              <div key={user.id} className={`relative ${index !== 0 ? "-left-1" : ""}`}>

                                <Link to={`/user/${user.id}`} className=" text-gray-700 hover:text-indigo-900">
                                  <img src={user.picture} alt={`User ${index}`} className="w-6 h-6 rounded-full border-gray-200 border transform hover:scale-125" />
                                </Link>
                      ...
                      <td className="py-3 px-6 text-center">
                        <div className="flex item-center justify-center">
                          <Link to={`/department/${depart.id}`} className="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
