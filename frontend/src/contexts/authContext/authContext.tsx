...
export default function AuthContextProvider({
    ...
    const [userInfo, setUserInfo] = useState<AuthContextType["userInfo"]>({} as AuthContextType["userInfo"]);
    ...
    const login = async (email: string, password: string, token?: string) => {
        const { data } = await axios.post('http://localhost:8007/login', { email, password }, {
            headers: {
    ...
    const register = async ({ email, name, password, surname }: RegisterDTO) => {
        ...
        await axios.post('http://localhost:8007/register', { email, password, name, surname }, {
            headers: {
                'Content-Type': 'application/json'
            },
        });
