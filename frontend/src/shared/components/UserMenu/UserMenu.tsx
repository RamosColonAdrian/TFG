...
import { UserInfo, authContext } from '../../../contexts/authContext/authContext'
...
export default function UserMenu({ user }: {
    ...
    const solutions = [
        { name: 'My Profile', description: 'Update profile info and picture', href: `/user/${user.id}`, icon: UserIcon },
    ]
    const callsToAction = [
    ...
    return (
        <Popover className="relative">
            <Popover.Button className="inline-flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900 outline-none">
                <img src={user.picture} className='w-10 h-10 rounded-full object-cover'/>
